...
class Mutual_Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        self.project_in = nn.Conv2d(2*dim, 2*dim, kernel_size=1, bias=bias)
...
class EventImage_ChannelAttentionTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias'):
        ...
        self.norm1_image = LayerNorm2d(2*dim)
...
class UNetConvBlock0(nn.Module):
    ...
    def forward(self, x, flow, enc=None, dec=None, mask=None, event_filter=None, merge_before_downsample=True):
        ...
        flow_resize2 = flow_resize.permute(0,2,3,1)
        event_filter_resize = event_filter.view(b*c//2, self.in_channels // c * 2, h, w)
        cond = flow_warp(event_filter_resize, flow_resize2).view(b,-1,h,w)
        # out1 = self.resblock2(out)
        out1 = self.image_event_transformer(out, self.resblock2(event_filter))
...
class BFNet(nn.Module):
    ...
    def forward(self, x, sharp_input, voxel_features, voxel_coordinates, batch_size, spatial_shapes, mask=None):
        ...
        for i, down in enumerate(self.down_path_1):
            ...
            if (i+1) < self.depth:
                x1, x1_up = down(x1, event_flow_list[i], event_filter=ev[i], merge_before_downsample=self.fuse_before_downsample)
                encs.append(x1_up)
                ...
                x1 = down(x1, event_flow_list[i], event_filter=ev[i], merge_before_downsample=self.fuse_before_downsample)
